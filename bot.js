...
// Configurações - NÚMERO DO ADMIN (apenas dígitos, sem + ou @)
const BOT_NUMBER = '556183040115';  // Número do bot (quem escanear o QR)
...
async function salvarQRCode(qr) {
    try {
        ...
        console.log('📱 QR Code salvo em: ./qr-code.png');
        console.log('🔗 Baixe o arquivo qr-code.png e escaneie!');
    } catch (err) {
...
async function connectToWhatsApp() {
    ...
    const sock = makeWASocket({
        ...
        printQRInTerminal: false, // Desabilita QR no terminal (vamos salvar como imagem)
        auth: state,
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        // ===== DEBUG: Mostra informações do remetente =====
        console.log('🔍 DEBUG - Sender completo:', sender);
        console.log('🔍 DEBUG - ADMIN_NUMBER configurado:', ADMIN_NUMBER);
        
        // Verifica se é admin - CORREÇÃO AQUI
        // O sender vem como: 5518997972598@s.whatsapp.net
        // Precisamos extrair apenas os dígitos para comparar
        const numeroLimpo = sender.replace('@s.whatsapp.net', '').replace('@g.us', '');
        console.log('🔍 DEBUG - Número limpo:', numeroLimpo);
        console.log('🔍 DEBUG - Comparação:', numeroLimpo, '===', ADMIN_NUMBER, '?', numeroLimpo === ADMIN_NUMBER);
        ...
        try {
            ...
            // COMANDO ADMIN - Verificação especial com logs
            if (text === 'admin' || text === 'adm') {
                console.log('🔍 DEBUG - Tentativa de acesso admin');
                console.log('🔍 DEBUG - isAdmin:', isAdmin);
                
                if (isAdmin) {
                    console.log('✅ Admin reconhecido! Abrindo menu admin...');
                    userStates.set(sender, { step: 'admin_menu' });
                    await sock.sendMessage(sender, { text: getMenuAdmin() });
                } else {
                    console.log('❌ Acesso negado! Número não é admin.');
                    await sock.sendMessage(sender, { 
